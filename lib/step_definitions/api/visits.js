...
When('the user with email {string} visited the space from establishment {string} {int} minutes ago with user data:', async function (email, establishment_name, minutes, dataTable) {
  ...
  try {
    ...
    const spaceId = this.establishments[establishment_name]['res']['body']['spaces'][0];
...
Then('the code generated by the user with email {string} is not in the billboard', function (email) {
  const codes = this.billboard.map(x => x['userGeneratedCode']);
  expect(codes).not.to.include(this.visits[email]['userGeneratedCode']);
});
...
const correctTypeFromKey = (key, value) => {
  ...
  if (['illnessRecovered'].includes(key)) {
    return value === 'true';
  }
  if (['vaccinatedDate', 'illnessRecoveredDate'].includes(key)) {

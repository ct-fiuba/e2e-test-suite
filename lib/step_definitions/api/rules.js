...
When('a new rule identified by the name {string} is created with fields:', async function (rule_name, dataTable) {
  ...
  try {
    ...
    this.rules[rule_name]['_id'] = res.body._id;
...
const correctTypeFromKey = (key, value) => {
  if (['index', 'durationValue', 'm2Value', 'vaccinated', 'vaccinatedDaysAgoMin', 'covidRecoveredDaysAgoMax'].includes(key)) {
    return parseInt(value);
  }
  if (['n95Mandatory', 'covidRecovered'].includes(key)) {
    return value === 'true';
  }
  return value;
}

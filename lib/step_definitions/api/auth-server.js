...
Then('the genux token for {string} is created', function (email) {
  assert.equal(this.users[email]['generateGenuxToken']['res'].statusCode, 201);
});

Then('the genux token for {string} is used correctly', function (email) {
  assert.equal(this.users[email]['useGenuxToken']['res'].statusCode, 204);
});

Then('the genux token for {string} is not used correctly', function (email) {
  assert.equal(this.users[email]['useGenuxToken']['res'].statusCode, 400);
});

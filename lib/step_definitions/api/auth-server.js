...
When('the user with email {string}, DNI {string} and password {string} is created', async function (email, dni, password) {
  ...
  const res = await this.http.post('signUp', { json: { email: email, DNI: dni, password: password }, responseType: 'json'});
...
Then('the {string} of {string} resulted in {string}', function(property, email, result) {
